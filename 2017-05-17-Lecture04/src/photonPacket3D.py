...
class PhotonPacket(object):
    ...
    def _updateXVar(self,r):
        """helper function

        running update of the  sum of squares of differences from the 
        current mean

        Notes:
            - part of the online algorithm for the sample variance from Ref [1]

        Refs:
            [1] https://en.wikipedia.org/wiki/Algorithms_for_calculating_variance
                online, accessed 2017-05-18
        """
        x, y, z = r
        xAv, yAv, zAv = self._xAv
        n = self.nSteps

        dx, dy, dz = x-xAv, y-yAv, z-zAv 
        xAv, yAv, zAv = xAv+dx/n, yAv+dy/n, zAv+dz/n 
        d2x, d2y, d2z = x-xAv, y-yAv, z-zAv 
        
        self._xM2 += dx*d2x  + dy*d2y + dz*d2z
        
    def Rg(self):
        """gyration radius 

        Returns:
            Rg (float) gyration radius of photon trail
        """
        return np.sqrt(self._xM2/(self.nSteps-1))

    def d(self):
        """displacement

        Implements displacement of the photon packet relativ to start

        Returns:
            d (float) end-to-end displacement of photon trail
        """
        x,y,z = self.x
        x0, y0, z0 = self.x0
